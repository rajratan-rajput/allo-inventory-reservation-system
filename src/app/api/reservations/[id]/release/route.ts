...
export async function POST(
  ...
  try {
    ...
    // Call transactional release logic
    const reservation = await ReservationService.releaseReservation(parsedId);

    // Safely serialize database results with relations
    const formatted = serializeReservation(reservation);

    return apiSuccess({
      message: 'Reservation successfully cancelled and stock released.',
